...
class UserService(
    ...
    fun signup(signupRequest: UserDto.SignupRequest): User {
        ...
        val newUser = User(
            ...
            signupRequest.name,
            encodedPassword,
            roles = "ROLE_" + signupRequest.role.uppercase() + ","
        )
    ...
    fun updateUser(user: User, updateRequest: UserDto.UpdateRequest): User {
        if (user.participantProfile != null) {
            ...
            user.participantProfile = user.participantProfile?.updatedBy(updateRequest)
                ?: throw UserNotFoundException("Participant Not Found")
        }

        if (user.instructorProfile != null) {
            user.instructorProfile =
                user.instructorProfile?.updatedBy(updateRequest) ?: throw UserNotFoundException("Instructor Not Found")
        }
